...
class BookingViewModel(private val userPreferences: UserPreferences) : ViewModel() {
    ...
    fun sendEmailToUserWhenBookAnEvent(
        name: String,
        startTime: String,
        endTime:String,
        date: String
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                message.setText(
                    "hello," + System.lineSeparator() +
                            "Booking completed successfully, you successfuly Book To Attent   $name   in  $date  at from $startTime to $endTime " + System.lineSeparator() + "$name Organizer Will contact you soon"
                )
                Transport.send(message)
                sendingMessageSuccess.postValue("Booking is Done Successfully You will Receive A Message Shortly ")
            } catch (e: MessagingException) {
